...
class PointsQuerySet(models.QuerySet):
    def weekly_results(self) -> List:
        ...
        # used this approach to order Points. E.g I wanted week's 1 results to
        # be order by who was 1st 2nd 3rd etc in current week
    ...
    def total_points_diff(self) -> List:
        ...
        current_total_leader_points = self.filter(
            week=current_week, current_leader=True)[0]
        return [
            f"{week_points.total_points:,d} [{current_total_leader_points.total_points - week_points.total_points:,d}]" for week_points in self.filter(
                week=current_week).order_by('-total_points')]
    ...
    def get_win_totals_with_overall_rank(self) -> List:
        ...
        for win_totals in WinTotals.objects.all():
            results.append(
                [win_totals, f'{overall_ranks.get(player=win_totals.player).overall_rank:,d}'])
        return results
...
class Points(models.Model):
    ...
    def weekly_winner(self):
        return ','.join([winner.player.displayed_name for winner in Points.objects.filter(
            week=self.week, max_points=True).order_by('player')])
