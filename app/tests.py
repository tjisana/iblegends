...
class EplFanstasyTestCase(TestCase):
    ...
    def test_get_current_event_and_status_from_web(self):
        ...
        httpretty.register_uri(httpretty.GET,
                               ...
                               body=json.dumps({"status": [{"bonus_added": True,
                                                            "date": "2020-11-27",
                                                            "event": 9,
                                                            "points": "r"},
                                                           {"bonus_added": True,
    ...
    def test_update_points_table_from_web_simple(self):
        ...
        player4_week1_points = Points.objects.get(
            week=1, player__displayed_name='P4')
        ...
        self.assertEqual(
            player4_week1_points.net_weekly_points,
            week1_points[3])
    ...
    def test_update_points_table_from_web_multiple_weeks(self):
        ...
        week2_total_points = [x + y - z * Points.TRANSFER_POINTS_COST for x, y, z in zip(week1_points, week2_points, transfers)]
        for player, point, transfer, week2_total in zip(
                Player.objects.all(), week2_points, transfers, week2_total_points):
            httpretty.register_uri(
                httpretty.GET,
                f"https://fantasy.premierleague.com/api/entry/{player.id}/event/{current_event}/picks/",
                body=json.dumps(
                    {
                        ...
                        "entry_history": {
                            "event": current_event,
                            "points": point,
                            "total_points": week2_total,
                            "event_transfers": transfer,
                            "event_transfers_cost": transfer * Points.TRANSFER_POINTS_COST,
                            "overall_rank": point}}))

        update_points_table_from_web(current_event, points_are_final)
        for point_in_db, week2_point_from_web, transfer in zip(Points.objects.filter(
                week=current_event).order_by('player__displayed_name'), week2_points, transfers):
    ...
    def test_update_weekly_winner(self):
        ...
        player4_wintotals = WinTotals.objects.get(
            player=Player.objects.get(displayed_name='P4'))
        self.assertEqual(player4_wintotals.weekly_wins, 1)
        self.assertEqual(player4_wintotals.winnings, WinTotals.WEEKLY_PRIZE)
        self.assertEqual(
            player4_wintotals.total_winnings,
            WinTotals.WEEKLY_PRIZE)
        ...
        current_event, points_are_final = get_current_event_and_status_from_web()
        week2_points = [49, 54, 51, 52]  # Week 2 points for player 1, 2, 3, 4
        transfers = [0, 1, 2, 0]  # Week 2 transfer_cost for player 1, 2, 3, 4
        week2_total_points = [x + y - z * Points.TRANSFER_POINTS_COST for x, y, z in zip(week1_points, week2_points, transfers)]
        ...
        self.assertEqual(
            Points.objects.get(
                week=2,
                player__displayed_name='P2').current_leader,
            False)
        self.assertTrue(
            Points.objects.get(
                week=2,
                player__displayed_name='P4').current_leader)
        self.assertTrue(
            Points.objects.get(
                week=2,
                player__displayed_name='P4').max_points)

        self.assertEqual(
            WinTotals.objects.get(
                player__displayed_name='P4').weekly_wins, 2)
        self.assertEqual(
            WinTotals.objects.get(
                player__displayed_name='P4').winnings,
            2 * WinTotals.WEEKLY_PRIZE)
    ...
    def test_update_points_table_from_web_multiple_weekly_winners(self):
                ...
                body=json.dumps(
                    {
                        ...
                        "entry_history": {
                            ...
                            "overall_rank": point}}))
        update_points_table_from_web(current_event, points_are_final)
        week1_player4_points = Points.objects.get(
            week=1, player__displayed_name='P4')
        week1_player3_points = Points.objects.get(
            week=1, player__displayed_name='P3')

        self.assertTrue(week1_player4_points.max_points)
        self.assertTrue(week1_player4_points.current_leader)
        self.assertEqual(week1_player4_points.weekly_winner, 'P3,P4')
        ...
        self.assertEqual(player4_wintotals.weekly_wins, 1)
        self.assertEqual(
            player4_wintotals.winnings,
            WinTotals.WEEKLY_PRIZE / 2)
        self.assertEqual(
            player4_wintotals.total_winnings,
            WinTotals.WEEKLY_PRIZE / 2)

        player3_wintotals = WinTotals.objects.get(
            player=Player.objects.get(displayed_name='P3'))
        self.assertEqual(player3_wintotals.weekly_wins, 1)
        self.assertEqual(
            player3_wintotals.winnings,
            WinTotals.WEEKLY_PRIZE / 2)
        self.assertEqual(
            player3_wintotals.total_winnings,
            WinTotals.WEEKLY_PRIZE / 2)
