...
def get_current_event_and_status_from_web():
    ...
    return response['status'][0]['event'], (all(
        statuses) and response['leagues'] != 'Updating')
    # return current event week. Also return check if points are final by
    # looking at status


def update_current_leader(week_number):
    max_total_points = -1000
    for player_total_score in Points.objects.filter(
            week=week_number).order_by('-total_points'):
        if player_total_score.total_points >= max_total_points:
            player_total_score.current_leader = True
            player_total_score.save()
            # this accomodates for the scenario that more than one player is
            # current leader
            max_total_points = player_total_score.total_points
        else:
            break
...
def update_weekly_max_points(week_number):
    ...
    for player_weekly_score in Points.objects.filter(
            week=week_number).order_by('-net_weekly_points'):
        if player_weekly_score.net_weekly_points >= max_net_weekly_points:
            ...
            # this accomodates for the scenario that more than one player has
            # max points
            max_net_weekly_points = player_weekly_score.net_weekly_points
        else:
            break


def update_win_totals(week_number):
    this_week_winners_points = Points.objects.filter(
        week=week_number, max_points=True)
    ...
    for points in this_week_winners_points:
        ...
        this_week_winner.winnings += decimal.Decimal(
            (WinTotals.WEEKLY_PRIZE / num_of_this_week_winners))
        this_week_winner.total_winnings += decimal.Decimal(
            WinTotals.WEEKLY_PRIZE / num_of_this_week_winners)
...
def update_points_table_from_web(current_event, points_are_final):
    ...
    for player in all_players:
        ...
        response = requests.get(url).json()
        Points.objects.create(**{'week': current_event,
                                 'player': player,
